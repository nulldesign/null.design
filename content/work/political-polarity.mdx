...
Ownership is original; the canonical repository is `willieavendano/political-polarity` on GitHub (link withheld from this page until its README, which still carries the earlier framing, is updated — a pull request is open). All commits are by the author, and no data is committed to the repository — the data directories are empty by design, with synthetic-data generation used for testing. Third-party components: scikit-learn, Hugging Face Transformers, Gradio.
